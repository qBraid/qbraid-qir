...
def test_map_qasm_op_to_pyqir_callable():
    ...
    for op_name in PYQIR_ONE_QUBIT_OP_MAP:
        callable_func, qubit_count = map_qasm_op_to_pyqir_callable(op_name)
        assert callable_func == PYQIR_ONE_QUBIT_OP_MAP[op_name]
        assert qubit_count == 1

    for op_name in PYQIR_ONE_QUBIT_ROTATION_MAP:
        callable_func, qubit_count = map_qasm_op_to_pyqir_callable(op_name)
        assert callable_func == PYQIR_ONE_QUBIT_ROTATION_MAP[op_name]
        assert qubit_count == 1

    for op_name in PYQIR_TWO_QUBIT_OP_MAP:
        callable_func, qubit_count = map_qasm_op_to_pyqir_callable(op_name)
        assert callable_func == PYQIR_TWO_QUBIT_OP_MAP[op_name]
        assert qubit_count == 2

    for op_name in PYQIR_THREE_QUBIT_OP_MAP:
        callable_func, qubit_count = map_qasm_op_to_pyqir_callable(op_name)
        assert callable_func == PYQIR_THREE_QUBIT_OP_MAP[op_name]

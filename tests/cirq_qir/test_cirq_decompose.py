...
def test_nested_custom_gates():
    ...
    class InnerCustomGate(cirq.Gate):
        def _num_qubits_(self):
            return 1
    ...
    class OuterCustomGate(cirq.Gate):
        def _num_qubits_(self):
            return 1
...
def test_qubit_mapping_to_line_qubits():
    """Test that arbitrary qubits are mapped to LineQubits."""
    ...
    # All qubits should be LineQubits
    for qubit in preprocessed_circuit.all_qubits():
...
def test_toffoli_gate():
    ...
    has_toffoli = any(
        isinstance(op.gate, cirq.CCXPowGate)
        for op in preprocessed_circuit.all_operations()
    )
    assert has_toffoli
...
def test_conditionally_controlled_operations():
    ...
    circuit = cirq.Circuit(
        ...
        cirq.X(qubit).with_classical_controls('m')
    )

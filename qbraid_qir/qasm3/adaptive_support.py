...
"""
Module defining Qasm3 Adaptive Visitor for QIR Adaptive Profile.
...
class QasmQIRAdaptiveVisitor:
    """A visitor for converting OpenQASM 3 programs to QIR using Adaptive Profile.

    This class is designed to traverse and interact with statements in an OpenQASM program
    with support for adaptive execution features like conditional branching on measurement
    results and qubit reuse after measurement.
    ...
    Args:
        ...
        emit_barrier_calls (bool): If True, barrier calls will be emitted. Defaults to False.
    """
    ...
    def _get_op_bits(self, operation: Any, qubits: bool = True) -> list[pyqir.Constant]:
        ...
        Returns:
            list[pyqir.Constant] : The bits for the operation.
        """
        qir_bits = []
        bit_list = []
    ...
    def _visit_barrier(self, barrier: qasm3_ast.QuantumBarrier) -> None:
        ...
        Args:
            barrier (qasm3_ast.QuantumBarrier): The barrier statement to visit.
        Returns:
            None
        """
    ...
    def _get_op_parameters(self, operation: qasm3_ast.QuantumGate) -> list[float]:
        ...
        if operation.arguments:
            for arg in operation.arguments:
                if isinstance(arg, qasm3_ast.FloatLiteral):
                    param_list.append(float(arg.value))
                elif isinstance(arg, qasm3_ast.IntegerLiteral):
                    param_list.append(float(arg.value))
                # Add other parameter types as needed
        return param_list

    def _visit_basic_gate_operation(self, operation: qasm3_ast.QuantumGate) -> None:
        """Visit a basic gate operation element using adaptive profile features.
    ...
    def _visit_branching_statement(self, statement: qasm3_ast.BranchingStatement) -> None:
        """Visit a branching statement element with adaptive profile support.
